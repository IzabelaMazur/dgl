...
CSRMatrix COOToCSR(COOMatrix coo) {
  ...
  NDArray ret_indptr_view = ret_indptr.CreateView({N}, coo.row->dtype, 1 * sizeof(IdType));
  NDArray ret_indices;
  NDArray ret_data;
  ...
  if (row_sorted) {
    // compute indptr
    IdType* Bp = static_cast<IdType*>(ret_indptr_view->data);
    Bp[0] = 0;
    int64_t j = 0;
    ...
    static_cast<IdType*>(ret_indptr->data)[0] = 0;
  }
